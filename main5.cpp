...
int main() {
    ...
    while (!WindowShouldClose()) { // Deteksi tombol tutup jendela atau tombol ESC
        gameScreen();
    BotUp  =leftPaddle.y > ball.y && (leftPaddle.y - ball.y) > 20 && ball.x < 400 && ball.x > 0; //Ditaruh disini agar bisa update kondisinya
    BotDown= leftPaddle.y < ball.y && (ball.y -leftPaddle.y) > 20 && ball.x < 400 && ball.x > 0; //Ditaruh disini agar bisa update kondisinya
    ...
    switch (currentScreen) {
        ...
        case TITLE: {
            ...
            if (IsKeyPressed(KEY_ENTER)) {
                framesCounter = 0;
                currentScreen = GAMEAI3; // Pindah ke layar GAMEPLAY saat ENTER ditekan
            }
            BeginDrawing();
        ...
        case GAMEPLAY1: {
            ...
            PaddleUp = IsKeyDown(KEY_W) && leftPaddle.y > 50;
            PaddleDown= IsKeyDown(KEY_S) && leftPaddle.y < GetScreenHeight() - 50;
